...
class _PatientFilesPageState extends State<PatientFilesPage> {
  ...
  Future<void> _performAddToWaitingList(
    ...
    final patientUid = patient['PATIENT_UID'] ?? patient['patientUid'] ?? patient['id'] ?? patient['IDNUMBER'] ?? 'UNKNOWN';
    final patientName = _getFullName(patient);
    final phone = patient['phone'] ?? patient['PHONE'] ?? '';
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // ✅ التعديل المهم: لا نقوم بجلب قائمة الانتظار بعد الإضافة
        // ❌ هذا ما كان يسبب المشكلة مع endpoint الـ GET
        // ✅ الاكتفاء برسالة النجاح فقط
  ...
  Future<void> _loadSecretaryDataToProvider() async {
    try {
      ...
      final image = (userData['IMAGE'] ?? prefs.getString('IMAGE') ?? '').toString().trim();
      final uid = (userData['USER_ID'] ?? userData['UID'] ?? userData['uid'] ?? prefs.getString('USER_ID') ?? '').toString();
      final email = (userData['EMAIL'] ?? prefs.getString('EMAIL') ?? '').toString();
      final phone = (userData['PHONE'] ?? userData['PHONENUMBER'] ?? userData['phoneNumber'] ?? prefs.getString('PHONE') ?? '').toString();

      secretaryProvider.setSecretaryData({
        'uid': uid,
        'fullName': fullName,
        'image': image,
        'email': email,
      ...
      debugPrint('Failed to load secretary data: $e');
    }
  }
