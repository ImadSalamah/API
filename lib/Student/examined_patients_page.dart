...
class DentalChartWidget extends StatelessWidget {
  ...
  Widget buildToothBox(ToothDisease tooth, bool isUpper, bool isEdge, double boxSize, double fontSize) {
    ...
    final Color boxColor = tooth.disease != null && tooth.disease!.trim().isNotEmpty
        ? StudentExaminedPatientsPage.primaryColor
        : Colors.grey[200]!;
    return SizedBox(
      width: boxSize,
      height: boxSize,
  ...
  Widget build(BuildContext context) {
    ...
    try {
      final inheritedState = context.findAncestorStateOfType<_StudentExaminedPatientsPageState>();
      if (inheritedState != null && inheritedState.widget.studentName != null) {
...
class _EnglishOnlyDelegate extends LocalizationsDelegate<MaterialLocalizations> {
  ...
  Future<MaterialLocalizations> load(Locale locale) => 
      DefaultMaterialLocalizations.load(Locale('en', 'US'));

  @override
  bool shouldReload(_EnglishOnlyDelegate old) => false;
}

class StudentExaminedPatientsPage extends StatefulWidget {
  ...
  const StudentExaminedPatientsPage({
    super.key,
    this.studentName,
  ...
  @override
  State<StudentExaminedPatientsPage> createState() => _StudentExaminedPatientsPageState();
}

class _StudentExaminedPatientsPageState extends State<StudentExaminedPatientsPage> {
  // API endpoints for Oracle database
  final String _baseApiUrl = ApiConfig.baseUrl;
  final String _studentExaminationsApi = '${ApiConfig.baseUrl}/student-examinations';
  ...
  final Map<String, String> _englishTranslations = {
    'examined_patients': 'My Assigned Patients',
    'name': 'Name',
    'phone': 'Phone',
    'age': 'Age',
    'no_patients': 'No assigned patients found',
  ...
  void initState() {
    ...
    _loadStudentExaminations();
    _searchController.addListener(_filterExaminations);
  }

  void _initializeStudentInfo() {
    setState(() {
      _studentName = widget.studentName;
      _studentImageUrl = widget.studentImageUrl;
      _currentUserId = widget.currentUserId;
      _allowedFeatures = widget.userAllowedFeatures ??
          ['examined_patients', 'add_patient', 'upload_xray'];
    });
  }

  Future<void> _loadStudentExaminations() async {
    try {
      setState(() {
        _isLoading = true;
        _hasError = false;
      ...
      debugPrint('🎓 Loading student examinations for: $_currentUserId');
      ...
      final response = await http.get(
        Uri.parse('$_studentExaminationsApi/$_currentUserId'),
      ...
      if (response.statusCode == 200) {
        ...
        if (completeExams.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('No examination data found for assigned patients'),
      ...
      debugPrint('❌ Error loading student examinations: $e');
      setState(() {
        _isLoading = false;
        _hasError = true;
      });
  ...
  Future<List<Map<String, dynamic>>> _getPrescriptionsForPatient(String? patientId) async {
    try {
      ...
      final Uri uri = Uri.parse('$_baseApiUrl/prescriptions/patient/$patientId');
  ...
  Future<List<Map<String, dynamic>>> _getAssignedStudentsForPatient(String? patientId) async {
    try {
      ...
      final Uri uri = Uri.parse('$_patientAssignmentsApi/$patientId');
    ...
    return DataTable(
      ...
      columns: const [
        ...
        DataColumn(
          ...
          tooltip: 'University ID of the student',
        ),
        DataColumn(
        ...
        Center(
          child: Column(
            children: [
              ...
              const SizedBox(height: 16),
              ElevatedButton(
                onPressed: _loadStudentExaminations,
  ...
  // ✅ Help dialog for ID image issues
  ...
  // ✅ Function to display ID image with zoom capability and better title
  ...
  // ✅ Function to display all available images for a patient
  ...
  // ✅ Helper function to build image thumbnail
  ...
  // ✅ Add this function to debug the actual API response
  ...
  // ✅ دالة لعرض الوصفات الطبية في جدول منظم
  ...
  // ✅ دالة لبناء جدول الوصفات الطبية
  ...
  // ✅ دالة لتنسيق تاريخ الوصفة الطبية
  ...
  // ✅ دالة لعرض الإجراءات السريرية في جدول منظم
  ...
  // ✅ دالة لبناء جدول الإجراءات السريرية
  ...
  Widget _buildSidebar() {
    return StudentSidebar(
      ...
      allowedFeatures:
          _allowedFeatures ?? ['examined_patients', 'add_patient', 'upload_xray'],
    );
  }
        ...
        children: [
          ...
          Expanded(
            child: _filteredExaminations.isEmpty
                ...
                : RefreshIndicator(
                    onRefresh: _loadStudentExaminations,
