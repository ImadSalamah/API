...
class DentalChartWidget extends StatelessWidget {
  ...
  Widget buildToothBox(ToothDisease tooth, bool isUpper, bool isEdge, double boxSize, double fontSize) {
    ...
    final Color boxColor = tooth.disease != null && tooth.disease!.trim().isNotEmpty
        ? _AdminExaminedPatientsPageState.primaryColor
        : Colors.grey[200]!;
    return SizedBox(
      width: boxSize,
      height: boxSize,
  ...
  Widget build(BuildContext context) {
    ...
    try {
      final inheritedState = context.findAncestorStateOfType<_AdminExaminedPatientsPageState>();
      if (inheritedState != null && inheritedState.widget.adminName != null) {
...
class AdminExaminedPatientsPage extends StatefulWidget {
  final String? adminName;
  final String? adminImageUrl;
  final String? currentUserId;
  final List<String>? userAllowedFeatures;

  const AdminExaminedPatientsPage({
    super.key,
    this.adminName,
    this.adminImageUrl,
    this.currentUserId,
  ...
  @override
  State<AdminExaminedPatientsPage> createState() => _AdminExaminedPatientsPageState();
}

class _AdminExaminedPatientsPageState extends State<AdminExaminedPatientsPage> {
  ...
  final String _patientAssignmentsApi = '${ApiConfig.baseUrl}/patient_assignments';
  ...
  // نظام الصلاحيات للإدمن فقط
  final Map<String, Map<String, dynamic>> _userPermissions = {
    'admin': {
      'canView': ['all_reports', 'all_patients', 'all_users'],
      'canEdit': ['all_reports'],
      'canDelete': [], // ✅ إزالة الحذف
      'canManageUsers': true,
    },
  };

  // دالة التحقق من الصلاحيات - مبسطة للإدمن فقط
  bool _hasPermission(String permissionType) {
    // ✅ الإدمن لديه جميع الصلاحيات دائماً
    return true;
  }
  ...
  void initState() {
    ...
    _initializeAdminInfo();
  }

  void _initializeAdminInfo() {
    // Set current user ID
    _currentUserId = widget.currentUserId;
    
    // تحميل معلومات الإدمن
    setState(() {
      _adminName = widget.adminName ?? 'Admin';
      _adminImageUrl = widget.adminImageUrl;
      _allowedFeatures = widget.userAllowedFeatures ?? ['all_reports', 'all_patients', 'all_users'];
    });
  }
  ...
  // =============================================
  // 🔥 PERMISSION CHECK METHODS - SIMPLIFIED FOR ADMIN
  // =============================================

  // ✅ SIMPLIFIED: Check if current user can edit the examination
  bool _canEditExamination(Map<String, dynamic> examination) {
    // ✅ الإدمن يمكنه تعديل كل الفحوصات
    return true;
  }

  // ✅ UPDATED: Function to navigate to edit examination page
      ...
      MaterialPageRoute(
        builder: (context) => Scaffold(
          appBar: AppBar(
            ...
            actions: [
              // ✅ UPDATED: Edit button with permission check and better styling
  ...
  // =============================================
  // 🔥 SIDEBAR METHODS - SIMPLIFIED FOR ADMIN
  // =============================================

  Future<void> _loadAllowedFeatures() async {
    // Features are now set in _initializeAdminInfo
  }
