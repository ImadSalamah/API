...
class AdminPrescriptionPage extends StatefulWidget {
  final String uid;
  const AdminPrescriptionPage({super.key, this.uid = ''});
...
class _AdminPrescriptionPageState extends State<AdminPrescriptionPage> {
  ...
  Future<void> _resolveCurrentAdminId() async {
    final languageProvider = Provider.of<LanguageProvider>(context, listen: false);
    final prefs = await SharedPreferences.getInstance();

    final providerId = languageProvider.currentUserId;
    ...
    if (userDataString != null) {
      try {
        ...
        userDataId = userData['USER_ID']?.toString() ?? userData['id']?.toString();
      } catch (_) {
        userDataId = null;
      }
    }
    ...
    if (_isMounted) {
      setState(() {
        _currentAdminId = widget.uid.isNotEmpty
            ? widget.uid
            : providerId ?? prefsId ?? userDataId;
  ...
  Future<void> _loadInitialData() async {
    try {
      await _resolveCurrentAdminId();
      await Future.wait([
        _fetchCurrentAdminName(),
        _loadDoctors(),
        _loadPatients(),
      ]);
    } finally {
  ...
  Future<void> _fetchCurrentAdminName() async {
    final targetId = _currentAdminId ?? widget.uid;
    if (targetId.isEmpty) {
      if (_isMounted) {
        _setUnknownAdmin();
      }
    ...
    try {
      final doctorResponse = await http.get(Uri.parse('$apiBaseUrl/doctors/$targetId'));
      ...
      if (doctorResponse.statusCode == 200) {
        ...
        if (_isMounted) {
          setState(() {
            _adminName = fullName;
            _adminImageUrl = doctorData['IMAGE']?.toString() ?? doctorData['image']?.toString();
      ...
      final userResponse = await http.get(Uri.parse('$apiBaseUrl/users/$targetId'));
      ...
      if (userResponse.statusCode == 200) {
        ...
        if (_isMounted) {
          setState(() {
            _adminName = fullName;
            _adminImageUrl = userData['IMAGE']?.toString() ?? userData['image']?.toString();
            _selectedDoctorName = _adminName;
            _selectedDoctorId ??= _currentAdminId;
          });
        }
  ...
  Future<void> _loadDoctors() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final normalized = _mapDoctors(data);
        if (_isMounted) {
          setState(() {
            _doctors = normalized;
          });
  ...
  List<Map<String, dynamic>> _mapDoctors(List<dynamic> data) {
    final Map<String, Map<String, dynamic>> unique = {};
    for (final raw in data) {
      if (raw is! Map) continue;
      final map = Map<String, dynamic>.from(raw);
      final id = map['DOCTOR_UID']?.toString().trim() ??
          map['USER_UID']?.toString().trim() ??
      ...
      unique.putIfAbsent(id, () => {'id': id, 'name': name});
    }
    return unique.values.toList();
  }

  String? _resolveDoctorIdForSubmit() {
    // 1) لو الإدمن موجود في doctors استخدمه
    final adminAsDoctor = _doctors.firstWhere(
      (d) => d['id']?.toString() == _currentAdminId,
      orElse: () => {},
    );
    if (adminAsDoctor.isNotEmpty) {
      _selectedDoctorId = adminAsDoctor['id'];
      _selectedDoctorName = adminAsDoctor['name'];
      return _selectedDoctorId;
    }
    // 2) وإلا استخدم أول طبيب متاح لضمان سلامة FK
    if (_doctors.isNotEmpty) {
      _selectedDoctorId = _doctors.first['id'];
      _selectedDoctorName = _doctors.first['name'];
      return _selectedDoctorId;
    }
    // 3) وإلا أعد null لنعلم أن الإدمن غير موجود كطبيب
    return null;
  }
  ...
  // لا يوجد اختيار طبيب؛ سيتم استخدام بيانات الأدمن نفسه
    ...
    if (_isMounted) {
      setState(() {
        ...
        if (selectedPatientIndex != null &&
            (selectedPatientIndex! < 0 || selectedPatientIndex! >= foundPatients.length)) {
          selectedPatientIndex = null;
        }
      });
    }
  ...
  Future<void> addPrescriptionToOracle() async {
    ...
    if (selectedPatientIndex! < 0 || selectedPatientIndex! >= foundPatients.length) return;
    final doctorId = _resolveDoctorIdForSubmit();
    if (doctorId == null || doctorId.isEmpty) {
      _showSnackBar('لا يمكن الحفظ: الحساب الحالي غير مسجل كطبيب ولا يوجد أطباء متاحون', isError: true);
      return;
    }
    ...
    for (final med in tempMedicines) {
      final prescriptionData = {
        ...
        'USAGE_TIME': med['time'] ?? '',
        'DOCTOR_NAME': _selectedDoctorName ?? _adminName ?? '',
        'DOCTOR_UID': doctorId,
  ...
  bool _isPrescriptionOwner(Map<String, dynamic> prescription) {
    ...
    final currentDoctorUid = _selectedDoctorId ?? _currentAdminId ?? widget.uid;
    return prescriptionDoctorUid.isEmpty || prescriptionDoctorUid == currentDoctorUid;
  }

  Future<void> deletePrescriptionFromOracle(String prescriptionId) async {
    try {
      final uri = Uri.parse(
        '$apiBaseUrl/prescriptions/$prescriptionId?doctorUid=${_selectedDoctorId ?? _currentAdminId ?? widget.uid}',
      );
      ...
      if (response.statusCode == 200) {
        if (selectedPatientIndex != null &&
            ...
            selectedPatientIndex! < foundPatients.length) {
          final foundPatient = foundPatients[selectedPatientIndex!];
          await fetchPatientPrescriptions(_getPatientIdNumber(foundPatient));
        }
  ...
  Future<void> updatePrescriptionInOracle(Map<String, dynamic> prescription) async {
    ...
    try {
      // استخدم نفس الطبيب المالك للوصفة لضمان عدم تعارض مع صلاحيات الـ API/ FK
      final effectiveDoctorUid = prescription['doctorUid']?.toString().isNotEmpty == true
          ? prescription['doctorUid'].toString()
          : _selectedDoctorId ?? _currentAdminId ?? widget.uid;
      final updateData = {
        ...
        'DOCTOR_NAME': _adminName ?? '',
        'DOCTOR_UID': effectiveDoctorUid,
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text(
                            selectedPatientIndex != null && selectedPatientIndex! >= 0 && selectedPatientIndex! < foundPatients.length
